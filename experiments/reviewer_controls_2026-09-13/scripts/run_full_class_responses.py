...
def response_batch(model, device, samples, category_index, text_sel, text_eval, logit_scale,
    ...
    return rows_out, eval_cat_raw.detach().cpu().numpy(), eval_cat_norm.detach().cpu().numpy(), device
...
def run(args: argparse.Namespace) -> None:
    ...
    with metadata_path.open(mode, newline="") as handle:
        ...
        for batch_no, (local_indices, images) in enumerate(loader):
            ...
            rows_out, batch_raw, batch_norm, current_device = response_batch(
    ...
    metadata = {
        ...
        "raw_response_definition": "mean over the three held-out prompts of logit-scale times masked-minus-original image feature dot class text feature",
